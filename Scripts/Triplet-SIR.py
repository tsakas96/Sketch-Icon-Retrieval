...
def train_step(sketches, positive_icons, negative_icons, margin, model):
    with tf.GradientTape() as tape:
        sketch_features,_ = tripleModel(sketches, training = True, model=model)
        icon_positive_features,_ = tripleModel(positive_icons, training = True, model=model) 
        icon_negative_features,_ = tripleModel(negative_icons, training = True, model=model)   
...
for epoch in range(num_epochs):
    ...
    if epoch%1==0:
        ...
        for j in range(0, len(sketches_Test), BATCH_SIZE):
            ...
            sketch_repr,_ =  tripleModel(sketches_array, training = False, model=choices[choice])
        ...
        for j in range(0, len(unique_icons_Test), BATCH_SIZE):
            ...
            icons_repr,_ =  tripleModel(icons_array, training = False, model=choices[choice])
        ...
        if top_acc < acc_1 + acc_10:
          save_weights(tripleModel, train_weights_path + "/tripletModel")
