...
algorithm = "Contrastive Loss-CWI" # CWI = Classification Weights Initialization
...
def train_step(sketches, icons, labels, margin):
    with tf.GradientTape(persistent=True) as tape:
        ...
        icon_features = iconClassificationModel(icons, training = True)[3]
...
for epoch in range(num_epochs):
    ...
    for i in range(0, len(pairs_Train), BATCH_SIZE):
        ...
        sketches, icons = get_batch_low_ram(batch_pairs)
